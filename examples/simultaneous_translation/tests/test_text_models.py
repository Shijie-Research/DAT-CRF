...
from fairseq.models import transformer_monotonic
...
class FakeTask(LegacyFairseqTask):
...
def make_sample_with_padding(longer_src=False) -> Dict[str, Any]:
    ...
    tokens_2 = torch.LongTensor(
        [[2, 11, 12, 13, 14, 2, PAD_INDEX, PAD_INDEX], [2, 11, 22, 33, 2, PAD_INDEX, PAD_INDEX, PAD_INDEX]],
    )
    if longer_src:
...
def build_transformer_monotonic_attention(**extra_args: Any):
    ...
    transformer_monotonic.monotonic_tiny_architecture(args)

    torch.manual_seed(0)
    task = FakeTask(args)
    return transformer_monotonic.TransformerModelSimulTrans.build_model(args, task)


def expected_alignment_formula(p_choose, mass_preservation=True, padding_mask=None):
    ...
    if padding_mask is not None:
        ...
        padding_mask = padding_mask.unsqueeze(1).expand([bsz_pad, num_heads, src_len]).contiguous().view(-1, src_len)
            ...
            for j in range(src_len):
                if i == 0:
                    if j == 0:
                        ...
                        # First target token
                        alpha[bsz_i, i, j] = p_choose[bsz_i, i, j] * torch.prod(1 - p_choose[bsz_i, i, :j])
                else:
                    alpha[bsz_i, i, j] = alpha[bsz_i, i - 1, j]
                    for k in range(j):
                        alpha[bsz_i, i, j] += alpha[bsz_i, i - 1, k] * torch.prod(1 - p_choose[bsz_i, i, k:j])
                    alpha[bsz_i, i, j] *= p_choose[bsz_i, i, j]

    alpha = alpha.masked_fill(padding_mask.unsqueeze(1), 0)

    if mass_preservation:
        alpha = mass_preservation_formula(alpha, False, padding_mask)

    return alpha


def mass_preservation_formula(alpha, left_padding=False, padding_mask=None):
    ...
    for bsz_i in range(bsz):
        if left_padding:
            ...
            alpha[bsz_i, :, src_lens[bsz_i] - 1] = 1 - alpha[bsz_i, :, : src_lens[bsz_i] - 1].sum(dim=-1)
...
class MonotonicAttentionTestAbstractClass(unittest.TestCase):
    def test_hard_aligned(self):
        configs = generate_config({"simul_type": "hard_aligned"})
        model = build_transformer_monotonic_attention(**configs)

        for longer_src in [True, False]:
            sample = make_sample_with_padding(longer_src)
            out, extra_out = model(**sample["net_input"])
            out.sum().backward()
            ...
            for item in extra_out.attn_list:
                ...
                alpha_real = expected_alignment_formula(
                    p_choose,
                    configs["mass_preservation"],
                    sample["net_input"]["src_tokens"].eq(PAD_INDEX),
                )

                self.assertTrue(torch.abs(alpha_system - alpha_real).le(5e-5).all())


class InfiniteLookbackTestCase(unittest.TestCase):
    def setUp(self):
        self.model = build_transformer_monotonic_attention(**generate_config({"simul_type": "infinite_lookback"}))
        self.model.train()
        ...
        for longer_src in [True, False]:
            ...
            for item in extra_out.attn_list:
                ...
                beta_real = expected_soft_attention_formula(
                    ...
                    soft_energy_system,
                    sample["net_input"]["src_tokens"].eq(PAD_INDEX),
                    chunksize=getattr(self.model.decoder.layers[0].encoder_attn, "chunk_size", int(1e10)) or int(1e10),
                )
        ...
        for longer_src in [True, False]:
            for k in [1, 3, 10, 20, 100]:
                ...
                for item in extra_out.attn_list:
                    ...
                    padding_mask = (
                        padding_mask.unsqueeze(1).expand([bsz, num_heads, src_len]).contiguous().view(-1, src_len)
                    )
