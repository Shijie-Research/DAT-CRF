...
def pretokenize(in_path: str, out_path: str, lang: str):
    Args = namedtuple(
        "Args",
        [
            "source_lang",
...
def _apply_bbpe(model_path: str, in_path: str, out_path: str):
    Args = namedtuple("Args", ["sentencepiece_model_path", "sentencepiece_enable_sampling", "sentencepiece_alpha"])
    args = Args(sentencepiece_model_path=model_path, sentencepiece_enable_sampling=False, sentencepiece_alpha=None)
...
def preprocess_iwslt17(
    ...
    for lang in [src, tgt]:
        for split in SPLITS:
            pretokenize(op.join(root, f"{split}.{lang}"), op.join(root, f"{split}.moses.{lang}"), lang=lang)
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        type=int,
        help="Generate tokenized bi-text with BPE of size K. Default to None (disabled).",
    )
    parser.add_argument(
        "--bbpe-vocab",
        default=None,
        type=int,
        help="Generate tokenized bi-text with BBPE of size K. Default to None (disabled).",
    )
    parser.add_argument("--byte-vocab", action="store_true", help="Generate tokenized bi-text with bytes vocabulary")
    parser.add_argument("--char-vocab", action="store_true", help="Generate tokenized bi-text with chars vocabulary")
