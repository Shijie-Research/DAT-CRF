...
class TEDIWSLT2017(Dataset):
    ...
    def _load(cls, task: str, distilled=False, sep=False, **kwargs):
        ...
        if not os.path.exists(binarized):
            ...
            data = datasets.load_dataset(cls.HF_PATH, pair=f"{lang1}-{lang2}", is_multilingual=False)
                ...
                if sep:
                    with open(os.path.join(data_dir, f"{name}.{lang1}"), "w") as f:
                        for text in tqdm(data[split]["translation"]):
                            f.write(" ".join(src_tokenizer.tokenize(text[lang1].lower())) + "\n")

                    with open(os.path.join(data_dir, f"{name}.{lang2}"), "w") as f:
                        for text in tqdm(data[split]["translation"]):
                            f.write(" ".join(tgt_tokenizer.tokenize(text[lang2].lower())) + "\n")
                else:
                    with open(os.path.join(data_dir, f"{name}.{lang1}"), "w") as f:
                        for text in tqdm(data[split]["translation"]):
                            f.write(" ".join(tokenizer.tokenize(text[lang1].lower())) + "\n")

                    with open(os.path.join(data_dir, f"{name}.{lang2}"), "w") as f:
                        for text in tqdm(data[split]["translation"]):
                            f.write(" ".join(tokenizer.tokenize(text[lang2].lower())) + "\n")
