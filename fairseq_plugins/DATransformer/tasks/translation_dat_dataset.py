...
class TranslationDATDataset(FairseqDataset):
    ...
    def __init__(
        ...
        assert not max_tokens_after_upsample or upsample_base in [
            "predict",
            "fixed",
        ], "--max-tokens-after-upsample should be used with upsample_base = predict or fixed."
        ...
        if self.upsample_base in ["source", "source_old", "fixed"]:
            ...
            if self.upsample_base == "source":
                ...
                length_src_special = torch.sum(
                    src_item.eq(self.src_dict.bos_index) | src_item.eq(self.src_dict.eos_index),
                    -1,
                )
                upsample_factor = (
                    np.random.rand() * (self.upsample_scale_max - self.upsample_scale_min) + self.upsample_scale_min
                )
                ...
                upsample_len = len(src_item) * upsample_factor  # compatible for older version
            elif self.upsample_base == "fixed":
                upsample_len = (
                    np.random.rand() * (self.upsample_scale_max - self.upsample_scale_min) + self.upsample_scale_min + 2
                )
            else:
            ...
            tgt_segid = torch.zeros(len(tgt_item), dtype=torch.long).fill_(
                -1,
            )  # the segment id for each token in target
            for i, tok in enumerate(tgt_item.numpy()):
                if self.tgt_dict.is_special_token(tok):
                    ...
                    if len(bound_pos) > 1:
                        ...
                        upsample_factor = (
                            np.random.rand() * (self.upsample_scale_max - self.upsample_scale_min)
                            + self.upsample_scale_min
                        )
            ...
            prev_output_tokens = torch.zeros(upsample_len, dtype=torch.long).fill_(
                self.tgt_dict.unk_index,
            )  # decoder input
            prev_output_tokens_segid = torch.zeros(
                upsample_len,
                dtype=torch.long,
            )  # the segment id for each token in decoder input
            force_emit = -torch.ones(
                upsample_len,
                dtype=torch.long,
            )  # forced aligned poisition for each token in decoder input
            bound_end = torch.zeros(
                upsample_len,
                dtype=torch.long,
            )  # segment end position for each token in decoder input
