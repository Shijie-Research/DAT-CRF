...
class TranslationDATConfig(TranslationConfig):
    seg_tokens: int = field(
        default=0,
        metadata={
            "help": "This parameter specifies the number of special tokens that will be used for segment id."
    ...
    max_tokens_after_upsample: bool = field(
        default=False,
        metadata={
            "help": "If enabled, the maximum number of tokens (--max-tokens) considered during generation "
    ...
    prepend_bos: bool = field(
        default=False,
        metadata={
            "help": "Prepends the beginning of sentence token (bos) to the source sequence. "
    ...
    decode_upsample_scale: Optional[float] = field(
        default=None,
        metadata={"help": "Upsampling scale to determine the DAG size during inference."},
    )
    ...
    decode_beamsize: float = field(default=100, metadata={"help": "Beam size used in beamsearch decoding."})
    decode_max_beam_per_length: float = field(
        default=10,
        metadata={"help": "Maximum number of beams with the same length in each step during beamsearch decoding."},
    )
    decode_gamma: float = field(
        default=0.1,
        metadata={
            "help": "Parameter used for n-gram language model score in beamsearch decoding. "
    ...
    do_not_load_task_args: bool = field(
        default=False,
        metadata={"help": "Do not load task arguments stored in checkpoints."},
    )
    noise: str = field(
...
class TranslationDATTask(TranslationTask):
    ...
    def build_model(self, cfg, from_checkpoint=False, reload_task=False):
        if reload_task:
            ...
            from fairseq.options import add_dataset_args, get_parser
            ...
            for key, value in vars(args).items():
                if hasattr(cfg, key) and getattr(cfg, key) != value:
                    logging.info(
                        f"Updating model arguments by user commands, key={key}, value_in_checkpoint={getattr(cfg, key)} -> value_in_cmd={value}",
                    )
                    setattr(cfg, key, value)
            # setting fp16
            if not getattr(args, "fp16", False) and hasattr(cfg, "fp16") and getattr(cfg, "fp16"):
                logging.info(
                    f"Updating model arguments by user commands, key=fp16, value_in_checkpoint=True -> value_in_cmd=False",
                )
                setattr(cfg, key, value)
        ...
        self.src_dict = TranslationDATDict.load(
            os.path.join(paths[0], "dict.{}.txt".format(self.cfg.source_lang)),
            seg_tokens=self.cfg.seg_tokens,
        )
        self.tgt_dict = TranslationDATDict.load(
            os.path.join(paths[0], "dict.{}.txt".format(self.cfg.target_lang)),
            seg_tokens=self.cfg.seg_tokens,
        )
    ...
    def build_dataset_for_inference(self, src_tokens, src_lengths, constraints=None):
        if constraints is not None:
            ...
            raise NotImplementedError("Constrained decoding with the translation_dat_task task is not supported")
    ...
    def inference_step(self, generator, models, sample, prefix_tokens=None, constraints=None, allow_future=False):
        with torch.no_grad():
    ...
    def get_batch_iterator(
        ...
        """
        can_reuse_epoch_itr = (
            not disable_iterator_cache and not update_epoch_batch_itr and self.can_reuse_epoch_itr(dataset)
        )
    ...
    def filter_indices_by_size_and_ratio(
        ...
        indices, ignored = filter_indices_by_size_and_ratio(
            indices,
            max_positions,
            filter_ratio if ignore_invalid_inputs else None,
        )
        if len(ignored) > 0:
