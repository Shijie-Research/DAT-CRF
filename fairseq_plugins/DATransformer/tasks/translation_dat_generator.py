...
class TranslationDATGenerator(object):
    ...
    def generate(self, models, sample, prefix_tokens=None, constraints=None, allow_future=False):
        if constraints is not None:
            raise NotImplementedError("Constrained decoding with the TranslationDATGenerator is not supported")
        ...
        if hasattr(decoder_out, "future"):  # for overlapped decoding
            if allow_future:
                return DecodeResult(
                    future=decoder_out.future,
                    fn=decoder_out.fn + [finish_hypo],
                    args=decoder_out.args + [(sent_idxs, self.pad, bsz)],
                )
            else:
