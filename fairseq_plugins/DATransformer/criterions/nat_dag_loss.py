...
class NATDAGLoss(FairseqCriterion):
    ...
    def __init__(self, cfg, task):
        ...
        self.torch_dag_loss = cfg.torch_dag_loss or cfg.torch_dag_operations
        self.torch_dag_logsoftmax_gather = cfg.torch_dag_logsoftmax_gather or cfg.torch_dag_operations
        self.torch_dag_best_alignment = cfg.torch_dag_best_alignment or cfg.torch_dag_operations
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            "For example, '0.5:0.1@200k' indicates annealing probability from 0.5 to 0.1 in 200k steps.",
        )
        parser.add_argument(
            "--glance-strategy",
            type=str,
            default=None,
            help='Set the glancing strategy. Possible values: "number-random" or "None" or "CMLM"',
        )
        parser.add_argument(
            "--no-force-emit",
            action="store_true",
            help="If true, the position of glanced tokens in the second forward pass will not be fixed.",
        )
        parser.add_argument(
            "--use-pretrain-loss",
            action="store_true",
            help="If true, use the pre-training loss, i.e. the position of segment id will be fixed.",
        )
        parser.add_argument(
            "--torch-dag-logsoftmax-gather",
            action="store_true",
        ...
        parser.add_argument(
            "--torch-dag-operations",
            action="store_true",
            help="Use torch implementation for all dag operations.",
        )
    ...
    def _compute_loss(self, outputs, targets, masks=None, label_smoothing=0.0, name="loss", factor=1.0):
        ...
        return {
            ...
            "nll_loss": nll_loss,
            "factor": factor,
            "ntokens": outputs.shape[0],
            "loss_nofactor": loss_nofactor,
        }
    ...
    def _force_aligment(self, match, force_emit):
        ...
        match_mask = torch.zeros(batch_size, tarlen + 1, prelen, dtype=torch.bool, device=match.device)
        match_mask = match_mask.scatter_(1, force_emit.unsqueeze(1) + 1, 1)[:, 1:]

        target_mask = match_mask.sum(dim=-1, keepdim=True) > 0
        match = match.masked_fill(target_mask, 0) + torch.zeros_like(match).masked_fill_(~match_mask, float("-inf"))
        match.masked_fill_(~target_mask, 0)
        ...
        for i in range(batch_size):
            ...
            for j in range(1, prelen):
                if force_emit_sample[j] >= 0 or j == prelen - 1:
                    ...
                    res[i] += (
                        torch_dag_loss(match_extract, links_extract, output_length, target_length)[0]
                        - match_extract[0, 0, 0]
                        - match_extract[0, -1, -1]
                    )
    ...
    def _compute_dag_loss(
        ...
        if self.torch_dag_logsoftmax_gather:
        ...
        if self.torch_dag_loss:
            ...
            assert (
                model.args.max_transition_length != -1
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if self.glat_p == 0:
            ...
            glat = {"context_p": max(self.glat_p, 0), "require_glance_grad": False}
        ...
        def glat_function(model, word_ins_out, tgt_tokens, prev_output_tokens, net_input, glat, links=None):
            ...
            if self.torch_dag_best_alignment:
                if model.args.max_transition_length != -1:
                    links = model.restore_valid_links(links)
                ...
                assert (
                    model.args.max_transition_length != -1
                ), "cuda dag best alignment does not support max_transition_length=-1. You can use a very large number such as 99999"
            ...
            if self.glance_strategy is None:
                keep_prob = (
                    (target_length - glat_prev_mask.sum(dim=-1) - same_num) / target_length * glat["context_p"]
                ).unsqueeze(-1) * predict_assigned_mask.float()
                ...
                glance_nums = ((target_length - glat_prev_mask.sum(dim=-1) - same_num) * glat["context_p"] + 0.5).to(
                    torch.long,
                )
                # prob_thresh = prob.topk(glance_nums.max().clip(min=1))[0].gather(-1, (glance_nums - 1).clip(min=0).unsqueeze(-1)).squeeze(-1)
                prob_thresh = (
                    prob.sort(descending=True)[0].gather(-1, (glance_nums - 1).clip(min=0).unsqueeze(-1)).squeeze(-1)
                )
                ...
                prob.masked_fill_(~predict_assigned_mask | glat_prev_mask, -100)
                glance_nums = (
                    (target_length - glat_prev_mask.sum(dim=-1)) * torch.rand_like(target_length, dtype=torch.float)
                    + 0.5
                ).to(torch.long)
                # prob_thresh = prob.topk(glance_nums.max().clip(min=1))[0].gather(-1, (glance_nums - 1).clip(min=0).unsqueeze(-1)).squeeze(-1)
                prob_thresh = (
            ...
            keep_word_mask = (
                torch.rand(prev_output_tokens.shape, device=prev_output_tokens.device) < keep_prob
            ).bool() | glat_prev_mask.squeeze(1)
        ...
        for l in losses:
            logging_output[l["name"]] = utils.item(l["loss_nofactor"]) if reduce else l["loss_nofactor"]
    ...
    def reduce_metrics(logging_outputs) -> None:
        ...
        sample_size = utils.item(sum(log.get("sample_size", 0) for log in logging_outputs))  # each batch is 1
        ...
        for key, value in res.items():
            metrics.log_scalar(key, value, sample_size, round=3)
