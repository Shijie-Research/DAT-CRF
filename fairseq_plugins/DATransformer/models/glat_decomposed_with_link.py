...
class GlatDecomposedLink(FairseqNATModel):
    ...
    def build_model(cls, args, task):
        ...
        if args.load_pretrained_model is not None:
                ...
                if states[position_name].size(0) < target_position_length:
                    ...
                    while states[position_name].size(0) < target_position_length:
                        ...
                        states[position_name] = torch.cat(
                            [states[position_name], expand_position_states[:, _index]],
                            dim=0,
                        )
                if states[position_name].size(0) > target_position_length:
                    logging.warning(
            ...
            if "encoder.layers.0.para" in states.keys():
                ...
                states = convert_state_from_ls_to_fs(
                    states,
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help="Path to a file containing a pre-trained model.",
        )

        parser.add_argument(
            "--links-feature",
            type=str,
            default="feature:position",
            help="Specifies the features used to predict transitions, separated by a colon. "
        ...
        try:
            ...
            parser.add_argument(
                "--decode-upsample-scale",
                type=float,
                default=None,
                help="Up-sampling scale to determine the DAG size during inference. "
            ...
            parser.add_argument(
                ...
                type=float,
                default=100,
                help="Beam size used in beamsearch decoding.",
            )
            parser.add_argument(
            ...
            parser.add_argument(
                ...
                type=bool,
                default=False,
                help="Enable token deduplication in BeamSearch.",
            )
        except:
            pass
    ...
    def extract_valid_links(self, content, valid_transition_mask):
        ...
        res = (
            content.masked_fill_(~valid_transition_mask.unsqueeze(-1), float("-inf"))
            .gather(
                2,
                valid_links_idx.clip(max=prelen - 1).unsqueeze(-1).expand(content.shape[0], -1, -1, content.shape[-1]),
            )
    ...
    def restore_valid_links(self, links):
        ...
        if translen > prelen - 1:
            translen = prelen - 1
        valid_links_idx = (
            torch.arange(prelen, dtype=torch.long, device=links.device).unsqueeze(1)
    ...
    def extract_links(
        ...
        if "position" in links_feature or "sinposition" in links_feature:
            links_feature_arr.append(
                link_positional(prev_output_tokens),
            )  # add learnable / absolute positional embeddings
        ...
        log_multi_content = torch.einsum(
            ...
            key_chunks.to(dtype=target_dtype),
        ) / (chunk_size**0.5)
        ...
        if (
            net_input is not None and "bound_end" in net_input and net_input["bound_end"] is not None
        ):  # if bound_end is prepared in net_input
        ...
        if self.args.max_transition_length != -1:  # finity transition length, prepare for cuda input format
            ...
            log_multi_content_extract = log_multi_content_extract.masked_fill(
        ...
        **kwargs,
        ...
        if glat and tgt_tokens is not None:
            with torch.set_grad_enabled(glat.get("require_glance_grad", False)):
                ...
                prev_output_tokens, tgt_tokens, glat_info = glat_function(
                    self,
                    word_ins_out,
                    tgt_tokens,
                    prev_output_tokens,
    ...
    def initialize_output_tokens(self, encoder_out, src_tokens, max_clip=None):
        ...
        if self.args.upsample_base == "source":
            ...
            length_src_special = torch.sum(
                src_tokens.eq(self.src_dict.bos_index) | src_tokens.eq(self.src_dict.eos_index),
                -1,
            )
            length_tgt = ((length_src - length_src_special) * self.args.decode_upsample_scale).long().clip_(
                min=0,
            ) + length_src_special
        ...
        if max_clip is not None and length_tgt.max() > max_clip:
            logging.warn(
                "clip predicted length... Try a smaller validation batch size, or use bigger max_decoder_batch_tokens",
            )
        ...
        initial_output_scores = initial_output_tokens.new_zeros(*initial_output_tokens.size()).type_as(
            encoder_out["encoder_out"][0] if "encoder_out" in encoder_out else encoder_out.encoder_out,
        )
    ...
    def max_positions(self):
        if vars(self.args).get("filter_max_length", None) is not None:
            ...
            return (
                min(self.encoder.max_positions(), int(self.decoder.max_positions() / self.args.decode_upsample_scale)),
                self.decoder.max_positions(),
            )
    ...
    def _analyze_graph(self, tgt_tokens, output_tokens, logits, links):
        ...
        word_ins_out, match = torch_dag_logsoftmax_gather_inplace(
            logits,
            tgt_tokens.unsqueeze(1).expand(-1, prelen, -1),
        )
    ...
    def forward_decoder(self, decoder_out, encoder_out, decoding_format=None, decoding_graph=False, **kwargs):
        ...
        prev_output_tokens_position = (
            torch.arange(seqlen, dtype=torch.long, device=output_tokens.device).unsqueeze(0).expand(bsz, -1) + 1
        ).masked_fill(output_tokens == self.tgt_dict.pad_index, 0)
        prev_output_tokens_segid = prev_output_tokens_position.masked_fill(
            output_tokens != self.tgt_dict.pad_index,
            1,
        ...
        net_input = {"prev_output_tokens_segid": prev_output_tokens_segid, "bound_end": bound_end}

        output_logits, links = self.extract_features(
            output_tokens,
            encoder_out,
            net_input,
            rand_seed,
    ...
    def inference_lookahead_simple(self, links, output_logits_normalized, output_length):
        ...
        if self.args.decode_strategy == "lookahead":
            links_idx = (
                (links + unreduced_logits.unsqueeze(1) * self.args.decode_beta).max(dim=-1)[1].cpu().tolist()
            )  # batch * prelen
    ...
    def inference_viterbi(self, links, output_logits_normalized, output_length):
        ...
        link_last = torch.gather(links, -1, (output_length - 1).view(batch_size, 1, 1).repeat(1, graph_length, 1)).view(
            1,
            batch_size,
            graph_length,
        )
    ...
    def inference_sample(self, links, output_logits_normalized, output_length):
        ...
        top_logits, top_logits_idx = output_logits_normalized.topk(self.args.decode_top_cand_n, dim=-1)
        dagscores_arr = (links / self.args.decode_temperature).log_softmax(dim=-1).unsqueeze(-1) + top_logits.unsqueeze(
            1,
    ...
    def inference(self, decoder_out, output_logits, links):
        ...
        if self.args.decode_strategy in ["lookahead", "greedy"]:
            if (
                self.args.decode_no_consecutive_repeated_ngram > 0
                or self.args.decode_no_repeated_ngram > 0
                and self.args.decode_strategy == "lookahead"
            ):
                inference_result = self.inference_lookahead_repeatprevent(
                    links,
                    output_logits_normalized,
                    output_length,
                )
            else:
            ...
            assert (
                self.args.decode_no_consecutive_repeated_ngram == 0 and self.args.decode_no_repeated_ngram == 0
...
class GlatLinkDecoder(NATransformerDecoder):
    ...
    def init_seg_feature(self, args, dictionary):
        self.embed_seg = PositionalEmbedding(
            dictionary.last_seg_token - dictionary.first_seg_token,
            args.decoder_embed_dim,
            0,
            True,
        )
    ...
    def forward_embedding(self, prev_output_tokens, prev_output_tokens_segid):
        ...
        if self.args.segment_embedding:
            x += F.embedding(
                prev_output_tokens_segid,
                self.embed_seg.weight,
                self.embed_seg.padding_idx,
                self.embed_seg.max_norm,
                self.embed_seg.norm_type,
                self.embed_seg.scale_grad_by_freq,
                self.embed_seg.sparse,
            )
    ...
    def extract_features(
        ...
        **unused,
        ...
        x, decoder_padding_mask = self.forward_embedding(prev_output_tokens, net_input["prev_output_tokens_segid"])
...
@register_model_architecture("glat_decomposed_link", "glat_decomposed_link_6e6d512")
...
@register_model_architecture("glat_decomposed_link", "glat_decomposed_link_base")
def base_architecture2(args):
    base_architecture(args)


@register_model_architecture("glat_decomposed_link", "glat_decomposed_link_pretrain")
...
@register_model_architecture("glat_decomposed_link", "glat_decomposed_link_big")
...
@register_model_architecture("glat_decomposed_link", "glat_decomposed_link_iwslt_de_en")
def glat_decomposed_link_iwslt_de_en(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
