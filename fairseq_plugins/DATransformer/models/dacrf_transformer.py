...
from ..criterions.utilities import parse_anneal_argument
...
@dataclass
class DACRFTransformerModelConfig(FairseqConfig):
...
class DACRFTransformerModel(NATransformerModel):
    def __init__(self, args, encoder, decoder):
        ...
        self.glat_scheduler = parse_anneal_argument(args.glance_p) if args.glance_p is not None else None
        ...
        self.query_linear = Linear(args.decoder.embed_dim * 2, args.decoder.embed_dim, bias=False)
        self.key_linear = Linear(args.decoder.embed_dim * 2, args.decoder.embed_dim, bias=False)
        self.gate_linear = Linear(args.decoder.embed_dim * 2, args.decoder.attention_heads, bias=False)

        if args.finetuning:
            for param in self.parameters():
                param.requires_grad = False

            self.embed_query = nn.Embedding(len(self.tgt_dict), args.crf_lowrank_approx, padding_idx=self.pad)
            self.embed_key = nn.Embedding(len(self.tgt_dict), args.crf_lowrank_approx, padding_idx=self.pad)
            self.crf_scaling = args.crf_lowrank_approx**-0.5
            self.crf_dropout = nn.Dropout(args.crf_dropout) if args.crf_dropout > 0 else nn.Identity()
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            help="Specifies the upsample base for the decoder input length in training.",
        )
        parser.add_argument(
            "--decode-strategy",
            type=str,
            default="viterbi",
            choices=["greedy", "lookahead", "viterbi", "full_viterbi", "sample", "beamsearch"],
        ...
        parser.add_argument("--finetuning", action="store_true", help="Only finetuning the CRF layer parameters.")
        parser.add_argument("--crf-dropout", type=float, default=0.0, help="Dropout rate for CRF layer.")
        parser.add_argument("--crf-lowrank-approx", type=int, default=32, help="Low-rank approximation for CRF layer.")
        parser.add_argument(
            "--crf-beam-approx",
            type=int,
            default=64,
            help="Beam size for apporixmating the normalizing factor",
        )
        parser.add_argument("--crf-decode-beam", type=int, default=8, help="Beam size for decoding")
    ...
    def _inference_full_viterbi(self, encoder_out, prev_output_tokens, target_tokens):
        ...
        for t in range(1, tgt_len):
            ...
            timestep_index = torch.where(
                target_mask[:, [t]][:, :, None, None],
                timestep_index,
                pre_eos_idx[:, :, None, None],
            )
        ...
        for beam_idx, timestep_idx in zip(reversed(traj_beam_index), reversed(traj_timestep_index)):
            ...
            timestep_idx = timestep_idx.gather(
                1,
                best_timestep[0][:, :, None, None].expand(-1, -1, beam, beam),
            ).squeeze(1)
