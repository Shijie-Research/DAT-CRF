...
__global__ void calculate_maxalpha_kernel(
    volatile int *bucket_queue,
    volatile int *accomplish_queue,
    Accessor1 alpha,
    Accessor2 trace,
    Accessor1 match_all,
    Accessor1 links,
...
void invoke_calculate_maxalpha(
    cudaStream_t stream,
    torch::Tensor &alpha,
    torch::Tensor &trace,
...
std::tuple<torch::Tensor, torch::Tensor> dag_best_alignment_op(const torch::Tensor &match_all, const torch::Tensor &links,
