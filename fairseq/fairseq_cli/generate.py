...
def _main(cfg: DictConfig, output_file):
    ...
    models, saved_cfg = checkpoint_utils.load_model_ensemble(
        ...
        strict=(cfg.checkpoint.checkpoint_shard_count == 1 and not cfg.checkpoint.no_strict_model_load),
                ...
                if not cfg.common_eval.quiet:
                    ...
                    if cfg.generation.print_alignment == "soft":
                        print(
                            "A-{}\t{}".format(
                                sample_id,
                                " ".join([",".join(src_probs) for src_probs in alignment]),
                            ),
                            file=output_file,
                        )
    ...
    if has_target:
        if cfg.bpe and not cfg.generation.sacrebleu:
            if cfg.common_eval.post_process:
                ...
                logger.warning(
                    "If you are using BPE on the target side, the BLEU score is computed on BPE tokens, not on proper words. "
...
def cli_main(input_args=None):
    ...
    parser.add_argument(
        ...
        metavar="ARCH",
        default="wav2vec2",
        help="Model architecture. For constructing tasks that rely on model args (e.g. `AudioPretraining`)",
    )
