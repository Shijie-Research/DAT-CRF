...
class FrozenBatchSampler:
    ...
    def make_batches_for_epoch(self, epoch, offset=0):
        self.batches = self.ordered_batches(epoch, self.fix_batches_to_gpus, self.shuffle)
        if offset > 0:
...
class GroupedIterator(CountingIterator):
    ...
    def __init__(self, iterable, chunk_size, skip_remainder_batch=False):
        if skip_remainder_batch:
            ...
            logger.info(f"skip final residual batch, grouped total_num_itrs = {total_num_itrs}")
        else:
...
class ShardedIterator(CountingIterator):
    ...
    def __init__(self, iterable, num_shards, shard_id, fill_value=None, skip_remainder_batch=None):
        """
        Args:
            ...
            if shuffle:
                with data_utils.numpy_seed(seed):
                    ...
                    if len(idx) * self.step_size != len(batches):
                        raise ValueError(
                            "ERROR: %d %d %d %d" % (len(idx), self.step_size, len(batches), self.shard_id),
