...
def _filter_by_size_dynamic(indices, size_fn, max_positions, raise_exception=False):
    ...
    def check_size(idx):
        if isinstance(max_positions, float) or isinstance(max_positions, int):
            ...
            return all(
                all(a is None or b is None or a <= b for a, b in zip(idx_size[key], max_positions[key]))
...
def filter_by_size(indices, dataset, max_positions, raise_exception=False):
    ...
    """
    warnings.warn(
        "data_utils.filter_by_size is deprecated. Use `FairseqDataset::filter_indices_by_size` instead.",
        stacklevel=2,
    )
    if isinstance(max_positions, float) or isinstance(max_positions, int):
...
def compute_block_mask_1d(
    ...
    if non_overlapping:
        ...
        mask = torch.nn.functional.conv_transpose1d(inp, w, stride=mask_length).squeeze(1)
        if mask.size(-1) > L:
            mask = mask[..., :L]
        ...
        mask_inds = torch.randint(
            ...
            size=(
                B,
                int(L * ((mask_prob + mask_prob_adjust) / mask_length) * (1 + mask_dropout)),
            ),
        )
...
def compute_mask_indices_v2(
    ...
    for i in range(bsz):
        ...
        mask_for_one = compute_mask_indices_for_one(sz, mask_prob, mask_length, seed, epoch, index, min_masks)
