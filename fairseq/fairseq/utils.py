...
def split_paths(paths: str, separator=os.pathsep) -> List[str]:
    return paths.split(separator) if "://" not in paths else paths.split(MANIFOLD_PATH_SEP)
...
def load_ensemble_for_inference(filenames, task, model_arg_overrides=None):
    ...
    deprecation_warning(
        "utils.load_ensemble_for_inference is deprecated. Please use checkpoint_utils.load_model_ensemble instead.",
    )
...
def extract_soft_alignment(attn, src_sent, tgt_sent, pad, eos):
    ...
    if len(tgt_valid) != 0 and len(src_valid) != 0:
        ...
        alignment = [["{:.6f}".format(p) for p in src_probs.tolist()] for src_probs in attn_valid]
    return alignment
...
def hotreload_function(name=None):
    ...
    * Requirements:
        ...
        1. pip install jurigged[develop]
    ...
    * Limitation:
    ...
    try:
        ...
        logger.warning("Please install jurigged: pip install jurigged[develop]")
        raise e
    import traceback
