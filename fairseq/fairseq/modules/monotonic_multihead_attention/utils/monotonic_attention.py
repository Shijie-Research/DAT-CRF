...
from .functions import moving_sum, prob_check


def expected_alignment_from_p_choose(p_choose: Tensor, padding_mask: Optional[Tensor] = None, eps: float = 1e-6):
    """
    ...
    Parallel solution:
    a_i = p_i * cumprod(1 − pi) * cumsum(a_i / cumprod(1 − pi))
    ...
    p_choose = p_choose.float().contiguous()
...
def expected_soft_attention(
    ...
    if chunk_size is not None:
        ...
        # Infinite lookback
        # Notice that infinite lookback is a special case of chunk-wise
        # where chunk-size = inf
...
def mass_preservation(alpha: Tensor, padding_mask: Optional[Tensor] = None, left_padding: bool = False):
    """
    Function to compute the mass preservation for alpha.
    ...
    if padding_mask is not None:
        if not left_padding:
            assert not padding_mask[
                :, 0
            ].any(), "Find padding on the beginning of the sequence. Please set left_padding to `True`"
        alpha = alpha.masked_fill(padding_mask.unsqueeze(1), 0.0)
