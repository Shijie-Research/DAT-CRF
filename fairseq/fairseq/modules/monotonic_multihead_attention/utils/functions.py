...
def exclusive_cumprod(tensor, dim: int, eps: float = 1e-10):
    ...
    tensor_size[dim] = 1
    return_tensor = safe_cumprod(torch.cat([torch.ones(tensor_size).type_as(tensor), tensor], dim), dim, eps)
...
def moving_sum(x, start_idx: int, end_idx: int):
    ...
    moving_sum = torch.nn.functional.conv1d(x, moving_sum_weight, padding=start_idx + end_idx - 1).squeeze(1)
