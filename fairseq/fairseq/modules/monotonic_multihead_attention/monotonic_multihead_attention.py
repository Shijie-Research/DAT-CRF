...
from .utils.p_choose_strategy import learnable_p_choose, waitk_p_choose
...
class MonotonicAttention(MultiheadAttention):
    ...
    def __init__(self, args):
        ...
        self.mass_preservation = not getattr(args, "no_mass_preservation", False)
        ...
        self.energy_bias = nn.Parameter(self.energy_bias_init * torch.ones([1])) if args.energy_bias else 0
    ...
    def add_args(parser):
        # fmt: off
        parser.add_argument(
            "--no-mass-preservation", action="store_true", help="Do not stay on the last token when decoding",
        )
        parser.add_argument(
            "--attention-eps", type=float, default=1e-6, help="Epsilon when calculating expected attention",
        )
        parser.add_argument("--noise-var", type=float, default=1.0, help="Variance of discreteness noise")
        parser.add_argument("--noise-mean", type=float, default=0.0, help="Mean of discreteness noise")
        parser.add_argument("--noise-type", type=str, default="flat", help="Type of discreteness noise")
    ...
    def monotonic_attention_process_infer(
        ...
        # 2. Update alpha
        alpha = p_choose.new_zeros([self.num_heads, src_len])
        alpha.scatter_(1, monotonic_step.view(self.num_heads, 1).clamp(0, src_len - 1), 1)
    ...
    def forward(
        ...
        assert attn_mask is None, "attn_mask must be None"
        assert all(x is not None for x in (query, key, value)), "query, key, and value must not be None"
        ...
        if key_padding_mask is not None:
            ...
            key_padding_mask = (
                key_padding_mask.unsqueeze(1).expand([bsz, self.num_heads, src_len]).contiguous().view(-1, src_len)
            )
...
class WaitKAttention(MonotonicInfiniteLookbackAttention):
    ...
    @staticmethod
    def add_args(parser):
        MonotonicInfiniteLookbackAttention.add_args(parser)
