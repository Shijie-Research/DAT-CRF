...
class MultiheadAttention(FairseqIncrementalDecoder):
    ...
    def __init__(
        ...
        xformers_blocksparse_layout: Optional[torch.Tensor] = None,  # This should be part of the config
        xformers_blocksparse_blocksize: Optional[int] = 16,  # This should be part of the config
    ):
    ...
    def forward(
        ...
        if attn_mask is not None:
            if attn_mask.ndim == 3:
                assert attn_mask.size(0) == bsz
                attn_mask = attn_mask.unsqueeze(1).repeat(1, self.num_heads, 1, 1).view(-1, tgt_len, src_len)
            else:
                    ...
                    if self.encoder_decoder_attention:
                        if input_buffer_k.size(0) * self.beam_size == new_order.size(0):
                            ...
                            input_buffer[k] = input_buffer_k.index_select(
                                0,
                                new_order.reshape(-1, self.beam_size)[:, 0] // self.beam_size,
                            )
                        else:
