...
class Trainer(object):
    ...
    def should_save_checkpoint_on_current_rank(self) -> bool:
        ...
        if (  # noqa
            ...
            or getattr(self.cfg.model, "base_layers", 0)
        ) > 0:
            return True
        else:
    ...
    def get_train_iterator(
        ...
        batch_iterator = self.task.get_batch_iterator(
            ...
            seed=(
                (self.cfg.common.seed + epoch) if self.cfg.dataset.update_ordered_indices_seed else self.cfg.common.seed
            ),
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        if self._sync_stats():
            ...
            ) = self._aggregate_logging_outputs(logging_outputs, sample_size, ooms, train_time, ignore=is_dummy_batch)
        ...
        with metrics.aggregate() as agg:
            ...
            if "loss" not in agg:
                if "loss" not in self._warn_once:
                    ...
                    logger.warning(
                        "Criterion.reduce_metrics did not log a 'loss' value, which may break some functionality",
                    )
