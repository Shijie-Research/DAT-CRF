...
def infer_init_method(cfg: DistributedTrainingConfig, force_distributed=False):
    ...
    if cfg.pipeline_model_parallel:
        ...
        if cfg.device_id > 0:
            logger.info("setting CUDA device={} on rank {}".format(cfg.device_id, cfg.distributed_rank))
    ...
    if node_list is not None:
        try:
            ...
            if ntasks_per_node == 1:
                ...
                assert (
                    ntasks_per_node == num_pipelines_per_node
                ), "SLURM --ntasks-per-node must match number of pipelines per node (={})".format(
...
def _infer_single_node_init(cfg: DistributedTrainingConfig):
    ...
    cfg.distributed_init_method = "tcp://localhost:{port}".format(port=cfg.distributed_port)
