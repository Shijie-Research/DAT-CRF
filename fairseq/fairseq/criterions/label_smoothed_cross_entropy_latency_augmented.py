...
class LabelSmoothedCrossEntropyCriterionLatencyAugmentConfig(LabelSmoothedCrossEntropyCriterionConfig):
    latency_avg_weight: float = field(
        default=0.0,
        metadata={"help": "weight for average latency loss."},
    )
    latency_var_weight: float = field(
        default=0.0,
        metadata={"help": "weight for variance latency loss."},
    )
    ...
    latency_var_type: str = field(
        ...
        metadata={"help": "latency type for variance loss"},
    )
...
class LatencyAugmentedLabelSmoothedCrossEntropyCriterion(LabelSmoothedCrossEntropyCriterion):
    ...
    def compute_latency_loss(self, model, sample, net_output):
        assert (
            net_output[-1].encoder_padding_mask is None or not net_output[-1].encoder_padding_mask[:, 0].any()
        ...
        src_lengths = (
            sample["net_input"]["src_lengths"].unsqueeze(1).expand(bsz, num_layers * num_heads).contiguous().view(-1)
        )
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        latency = utils.item(sum(log.get("latency", 0) for log in logging_outputs))
        delays_var = utils.item(sum(log.get("delays_var", 0) for log in logging_outputs))
        latency_loss = utils.item(sum(log.get("latency_loss", 0) for log in logging_outputs))
