...
class RobertaEncDecModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            action="store_true",
            help="hack to reload old models trained with encoder-normalize-before=False "
            "(no equivalent to encoder-normalize-before=False and layernorm_embedding=False",
        )
        parser.add_argument(
    ...
    def build_model(cls, args, task):
        ...
        if args.pretrained_mlm_checkpoint:
            ...
            loaded = fairseq.checkpoint_utils.load_model_ensemble_and_task(
                [args.pretrained_mlm_checkpoint],
    ...
    def from_roberta(roberta_enc: roberta.RobertaModel, args, dictionary):
        ...
        if args.share_all_embeddings:
            ...
            assert encoder.embed_tokens.weight is lm_head.weight, (
                "Can't use --share-all-embeddings with a model " "that was pretrained with --untie-weights-roberta_enc"
            )
        else:
        ...
        if args.share_all_embeddings or args.share_decoder_input_output_embed:
            # Note: I wasn't able to use Embedding _weight parameter to archive this sharing.
        ...
        if getattr(args, "pretrained_decoder", False):
            ...
            assert not missing_keys and not unexpected_keys, (
                "Failed to load state dict. " f"Missing keys: {missing_keys}. " f"Unexpected keys: {unexpected_keys}."
            )
