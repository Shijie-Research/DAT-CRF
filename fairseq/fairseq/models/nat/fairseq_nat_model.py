    ...
    def wrapper(self, *args, **kwargs):
        ...
        if num_layers > 0:
            _encoder_out["encoder_states"] = [
                torch.stack([e["encoder_states"][i] for e in encoder_outs], -1) for i in range(num_layers)
            ]
        return _encoder_out
    ...
    def wrapper(self, normalize=False, encoder_out=None, *args, **kwargs):
        ...
        action_outs = [
            func(
                model,
                normalize=normalize,
                encoder_out=_replace(encoder_out, encoder_out["encoder_out"][0][:, :, :, i]),
                *args,
                **kwargs,
            )
        ...
        for i in range(len(action_outs[0])):
            if i == 0 and normalize:
                ensembled_outs += [
                    torch.logsumexp(torch.stack([a[i] for a in action_outs], -1), dim=-1)
