...
def _get_ins_targets(in_tokens, out_tokens, padding_idx, unk_idx):
    ...
    def _get_ins_targets_cuda(in_tokens, out_tokens, padding_idx, unk_idx):
        ...
        mask_ins_targets = mask_ins_targets.type_as(in_tokens)[:, 1 : in_masks.size(1)].masked_fill_(
            ~in_masks[:, 1:],
            0,
        )
    ...
    def _get_ins_targets_cpu(in_tokens, out_tokens, padding_idx, unk_idx):
        ...
        for mask_input in mask_inputs:
            ...
            masked_tgt_masks.append(mask_label + [0 for _ in range(out_seq_len - len(mask_label))])
        mask_ins_targets = [
