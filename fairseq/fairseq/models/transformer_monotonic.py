...
from fairseq.modules.monotonic_transformer_layer import (
...
class TransformerMonotonicEncoder(TransformerEncoder):
    def build_encoder_layer(self, cfg, layer=None):
        layer = TransformerMonotonicEncoderLayer(cfg)
        return super().build_encoder_layer(cfg, layer=layer)
...
class TransformerMonotonicDecoder(TransformerDecoder):
    ...
    def build_decoder_layer(self, cfg, no_encoder_attn=False, layer=None):
        layer = TransformerMonotonicDecoderLayer(cfg)
        return super().build_decoder_layer(cfg, no_encoder_attn=no_encoder_attn, layer=layer)
    ...
    def pre_attention(
        ...
        if "encoder_padding_mask" in encoder_out_dict and len(encoder_out_dict["encoder_padding_mask"]) > 0:
            encoder_padding_mask = encoder_out_dict["encoder_padding_mask"][0]
        else:
