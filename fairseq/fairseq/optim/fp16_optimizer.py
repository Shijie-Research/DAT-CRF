...
class _FP16OptimizerMixin(object):
    ...
    def has_flat_params(self):
        return torch.is_tensor(self.fp32_params) or (
            isinstance(self.fp32_params, dict) and all(torch.is_tensor(t) for t in self.fp32_params.values())
        )
    ...
    def build_fp32_params(cls, args, params, flatten=True):
        ...
        if flatten:
            is_pipeline_parallel = getattr(args, "pipeline_model_parallel", False) and getattr(
                args,
