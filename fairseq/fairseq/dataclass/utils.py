...
def override_module_args(args: Namespace) -> Tuple[List[str], List[str]]:
    ...
    for k in FairseqConfig.__dataclass_fields__.keys():
        overrides.extend(_override_attr(k, FairseqConfig.__dataclass_fields__[k].type, args))

    if args is not None:
        if hasattr(args, "task"):
            from fairseq.tasks import TASK_DATACLASS_REGISTRY

            migrate_registry("task", args.task, TASK_DATACLASS_REGISTRY, args, overrides, deletes)
        else:
            deletes.append("task")
...
def overwrite_args_by_name(cfg: DictConfig, overrides: Dict[str, any]):
    ...
    with open_dict(cfg):
        for k in cfg.keys():
            # "k in cfg" will return false if it is a "mandatory value (e.g. ???)"
