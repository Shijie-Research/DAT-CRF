...
class CommonConfig(FairseqDataclass):
    ...
    aim_run_hash: Optional[str] = field(
        default=None,
        metadata={"help": "Aim run hash. If skipped, creates or continues run based on save_dir"},
    )
...
class DistributedTrainingConfig(FairseqDataclass):
    ...
    distributed_init_method: Optional[str] = field(
        default=None,
        metadata={"help": "typically tcp://hostname:port that will be used to establish initial connection"},
    )
    ...
    find_unused_parameters: bool = field(
        default=False,
        metadata={"help": "disable unused parameter detection (not applicable to --ddp-backend=legacy_ddp)"},
    )
    gradient_as_bucket_view: bool = field(
        default=False,
        metadata={
            "help": "when set to True, gradients will be views pointing to different offsets of allreduce communication buckets. "
            "This can reduce peak memory usage, where the saved memory size will be equal to the total gradients size. "
    ...
    heartbeat_timeout: int = field(
        default=-1,
        metadata={"help": "kill the job if no progress is made in N seconds; set to -1 to disable"},
    )
    broadcast_buffers: bool = field(
        default=False,
        metadata={"help": "Copy non-trainable parameters between GPUs, such as batchnorm population statistics"},
    )
...
class DatasetConfig(FairseqDataclass):
    ...
    max_tokens_valid: Optional[int] = field(
        ...
        metadata={"help": "maximum number of tokens in a validation batch (defaults to --max-tokens)"},
    )
    ...
    update_ordered_indices_seed: bool = field(
        default=False,
        metadata={"help": "if true then increment seed with epoch for getting batch iterators, defaults to False."},
    )
...
class CheckpointConfig(FairseqDataclass):
    ...
    restore_file: str = field(
        ...
        metadata={"help": "filename from which to load checkpoint (default: <save-dir>/checkpoint_last.pt"},
    )
    ...
    no_strict_model_load: bool = field(
        default=False,
        metadata={"help": "allow missing or unexpected keys when loading from a pretrained model"},
    )
...
class EvalLMConfig(FairseqDataclass):
    output_word_probs: bool = field(
        default=False,
        metadata={"help": "if set, outputs words and their predicted log probabilities to standard output"},
    )
    output_word_stats: bool = field(
        default=False,
        metadata={"help": "if set, outputs word statistics such as word count, average probability, etc"},
    )
...
class EMAConfig(FairseqDataclass):
    ...
    ema_seed_model: Optional[str] = field(
        default=None,
        metadata={"help": "Seed to load EMA model from. Used to load EMA model separately from the actual model."},
    )
