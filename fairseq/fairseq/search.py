...
class DiverseBeamSearch(Search):
    ...
    Using diversity_discount to interpolate between the two:
    if diversity_discount = 0.5, then
    ...
    NB: During beam search for each diversity group, `candidate_multiple` is set to 1 rather than BeamSearch default(2).
    ...
    def step(
        ...
        for g in range(self.num_groups):
            ...
            if g > 0:
                ...
                diversity_buf.scatter_add_(
                    ...
                    penalty_val.expand(indices_.size()).reshape(bsz, -1).to(diversity_buf),
                )
            ...
            scores_buf, indices_buf, beams_buf = self.beam.step(
                ...
                lprobs_g,
                scores_g,
                candidate_multiple=self.candidate_multiple,
            )
        ...
        overlap = self.diversity_discount * torch.sum(
            indices_G_stacked.unsqueeze(2).eq(indices_G_stacked.unsqueeze(3)),
            dim=1,
        )
        if step == 0:
            ...
            self.group_overlap[original_batch_idxs] = (
                self.group_overlap[original_batch_idxs] * self.diversity_discount + overlap
            )
...
class Sampling(Search):
    ...
    def _sample_topp(self, lprobs):
        ...
        Return: A tuple of (trimmed_probs, truncated_indices) where:
            trimmed_probs: (bsz x input_beam_size x ?)
