...
def last_n_best_checkpoints(paths, n, metric, maximize_metric):
    assert len(paths) == 1
    path = paths[0]
    ...
    if len(entries) < n:
        raise Exception(f"Found {len(entries)} checkpoint files but need at least {n}")
    return [os.path.join(path, x[1]) for x in sorted(entries, reverse=True if maximize_metric else False)[:n]]
